...
dataport_get_nth_frame_cap(dataport_caps_handle_t *d, unsigned int n)
{
    return d->get_nth_frame_cap(n);
}

static inline unsigned int
dataport_get_id(dataport_caps_handle_t *d)
{
    return d->get_id();
}

static inline unsigned int
dataport_get_num_frame_caps(dataport_caps_handle_t *d)
{
    return d->get_num_frame_caps();
}

static inline seL4_CPtr*
dataport_get_frame_caps(dataport_caps_handle_t *d)
{
    return d->get_frame_caps();
}

static inline void
dataport_free_frame_caps(dataport_caps_handle_t *d, seL4_CPtr *frame_caps)
{
