...
unsigned int vfat_striptail_len(const struct qstr *qstr)
{
...
int vfat_build_slots(struct inode *dir, const unsigned char *name,
			  ...
			  struct fat_slot_info *sinfo)
{
	...
	if (len == 0)
		return -ENOENT; // why is this negative
...
static int __init init_vfat(void)
{
        int err = 0;
...
static void __exit exit_vfat(void)
{
...
module_init(init_vfat)
module_exit(exit_vfat)

MODULE_AUTHOR("Data61 TFS Team");
MODULE_DESCRIPTION("VFAT implementation in Cogent");
MODULE_LICENSE("GPL");
